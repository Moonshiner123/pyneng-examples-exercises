        ...
        for row in data_dhcp:
            ...
            if db_entry:
                ...
                for entry in db_entry:
                    curr_mac, curr_ip, curr_vlan, curr_interface, curr_switch, curr_active_flag = entry
                ...
                if new_row == curr_row:
                    ...
                    update_query = "UPDATE dhcp set active = ? WHERE mac = '{}'".format(mac)
                    ...
                    replace_query = '''REPLACE INTO dhcp  values (?, ?, ?, ?, ?, ?)'''
                ...
                '''
                Если строка в БД с таким же маком найдена, делаем новую запись в БД.
                '''
                #active_flag = 1
                print('Оппачки! Такого мака еще не было! Создаю новую запись')
                insert_query_dhcp = '''insert into dhcp (mac, ip, vlan, interface, switch, active) values (?, ?, ?, ?, ?, ?)'''
                curr_row = (mac, ip, vlan, interface, switch, active_flag)
